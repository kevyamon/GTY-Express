...
export const sendOrderConfirmationEmail = (order, user) => {
  const subject = `✅ Commande confirmée #${order._id.toString().substring(0, 8)}`;
  const htmlContent = `
    <html>
      <body>
        <h1>Bonjour ${user.name}, 👋</h1>
        <p>Merci pour votre commande sur GTY Express !</p>
        <p>Nous avons bien reçu votre commande <strong>#${order._id.toString().substring(0, 8)}</strong> d'un total de <strong>${order.totalPrice.toFixed(2)} FCFA</strong>.</p>
...
export const sendStatusUpdateEmail = (order, user) => {
  const subject = `🚚 Mise à jour de votre commande #${order._id.toString().substring(0, 8)}`;
  const htmlContent = `
    <html>
      <body>
        <h1>Bonjour ${user.name},</h1>
        <p>Bonne nouvelle ! Le statut de votre commande <strong>#${order._id.toString().substring(0, 8)}</strong> a été mis à jour.</p>
